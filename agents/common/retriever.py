...
__all__ = ["VultronRetriever", "Retriever", "IngestSummary"]
...
class VultronRetriever:
    ...
    async def ensure_collection(self) -> str:
        ...
        Idempotent: matches an existing collection by name (the API derives the
        id from the name, so we always read the id it returns rather than assume
        id == name).
        """
        ...
        resp = await self._client.post(_VECTOR_STORE, json={"name": self.collection_name})
        resp.raise_for_status()
        self._collection_id = resp.json()["collection"]["id"]
        return self._collection_id
